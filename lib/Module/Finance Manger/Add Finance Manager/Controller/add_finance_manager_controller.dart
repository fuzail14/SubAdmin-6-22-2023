  ...
  financeManagerRegisterApi(
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < errorList.length; i++) {
        showDialogBox(context, title: 'Operation Failed.', data: errorList[i]);
        clearText();
      }
    } else if (response.statusCode == 500) {
      isLoading = false;
      update();
      showDialogBox(context, title: 'Operation Failed.', data: 'Sever Error');
      clearText();
    } else {
      showDialogBox(context,
          title: 'Operation Failed.', data: 'Something Went Wrong');
      clearText();
    }
  }
